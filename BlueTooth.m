...
- (void)printStringWithContent:(NSString *)string
{
    ...
    if (blueTooth.peripheral && blueTooth.manager.state == 5 && blueTooth.peripheral.state == 2) {
        ...
        MervinBlueToothPrinterViewController * blueToothVC = [[MervinBlueToothPrinterViewController alloc] init];
...
#pragma mark -- 连接上回调－－但不能确定是否一定成功，还需要在里面多次重新连接
